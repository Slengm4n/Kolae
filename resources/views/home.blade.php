    ...
    <div class="container mx-auto px-4 flex justify-between items-center">
        ...
        <div class="flex items-center gap-4">
            ...
            <div class="relative z-50">
                <button id="lang-btn"
                    class="flex items-center justify-center w-10 h-10 rounded-full bg-surface-elevated/50 backdrop-blur-sm border border-gray-700/50 hover:bg-surface-elevated transition-all text-white">
                    <i class="fas fa-globe"></i>
                </button>

                <div id="lang-modal"
                    class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 opacity-0 invisible transition-all duration-300 ease-in-out">
                    <div id="lang-box"
                        class="bg-[#1e293b] border border-gray-700/50 text-white rounded-2xl shadow-2xl w-72 p-6 transform scale-95 opacity-0 transition-all duration-300 ease-in-out">
                        ...
                        <ul class="space-y-2 font-medium">
                            @foreach([
                                ...
                                ['lang' => 'hi-in', 'flag' => 'HI',   'label' => 'मानक हिन्दी'],
                                ['lang' => 'zh-cn', 'flag' => 'ZH',   'label' => '官话'],
                                ['lang' => 'it-it', 'flag' => 'IT',   'label' => 'Italiano'],
                                ['lang' => 'ja-jp', 'flag' => 'JA',   'label' => '日本語'],
                            ] as $item)
                            <li>
                                <a href="{{ route('lang.switch', $item['lang']) }}"
                                   class="block w-full text-left px-4 py-3 rounded-xl bg-gray-800/50 hover:bg-cyan-500/20 hover:text-cyan-400 transition-all duration-200 flex items-center gap-3 hover:scale-105 active:scale-95">
                                    <span class="text-2xl">{{ $item['flag'] }}</span> {{ $item['label'] }}
                                </a>
                            </li>
                            @endforeach
                        </ul>
...
<main>
    ...
    <section class="relative h-screen flex items-center justify-center text-center md:justify-start md:text-left p-0 overflow-hidden">
        ...
        <video autoplay muted loop playsinline class="vimeo-bg-cover">
            <source src="{{ asset('assets/img/hero-bg.mp4') }}" type="video/mp4">
        </video>
    ...
    <div class="container mx-auto px-4 grid md:grid-cols-2 lg:grid-cols-3 gap-12">
        ...
        <div class="mb-8 text-center md:text-left">
            ...
            <form action="{{ route('newsletter.subscribe') }}" method="POST" class="flex mt-4">
                ...
                <input type="email"
                       name="email"
                       placeholder="{{ __('home_footer_email_placeholder') }}"
                       required
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    langBtn?.addEventListener('click', e => { e.stopPropagation(); openModal(); });
    closeLangBtn?.addEventListener('click', e => { e.stopPropagation(); closeModal(); });
    langModal?.addEventListener('click', e => { if (!langBox.contains(e.target)) closeModal(); });
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && langModal?.classList.contains('visible')) closeModal();
    });
