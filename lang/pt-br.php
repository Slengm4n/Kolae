<?php
return [
    //Chaves globais
    'global_slogan_headline' => 'Cole com quem ama esporte',
    'global_menu_about' => 'Sobre Nós',
    'global_title_about' => 'Nossa Missão é Conectar Atletas',
    'global_text_about' => 'Acreditamos que o esporte tem o poder de unir pessoas, criar comunidades e transformar vidas. A Kolae nasceu do desejo de facilitar essa conexão, oferecendo uma plataforma onde atletas de todos os níveis podem encontrar parceiros de treino, equipes e eventos na sua região.',
    'global_button_language' => 'Selecione a linguagem',
    'global_contact_kolae' => 'Contato',
    'global_Copyright_message' => '2025 Kolae. Todos os direitos reservados',
    'global_home_panel' => 'Meu Painel',
    'global_menu_profile' => 'Perfil',
    'global_menu_exit' => 'Sair',
    'global_menu_register' => 'Cadastre-se',
    'global_menu_login' => 'Entrar',
    'global_and' =>  'e',
    'global_menu_help' => 'Ajuda',
    'global_menu_config' => 'Configurações',
    'global_cancel' => 'Cancelar',
    'global_back' => 'Voltar',
    'global_error' => 'Erro',
    'global_account' => 'Conta',


    //global Forms
    'global_email' => 'Email',
    'global_senha' => 'Senha',

    //Home

    'perfil' => 'perfil',
    'home_start_free' => 'Comece de graça',
    'home_connect_message' => 'Conecte-se com quem compartilha a sua paixão pelo esporte.',
    'home_search_message' => 'Encontre atletas com os mesmos interesses, nível de habilidade e na sua região. Mais do que uma conexão, é o seu novo time.',
    'home_title_community' => 'Comunidade',
    'home_text_community' => 'Construa seu time e faça parte de uma rede de atletas apaixonados.',
    'home_title_location' => 'Conexão Local',
    'home_text_location' => 'Encontre treinos e jogos perto de você, a qualquer hora.',
    'home_title_supporters' => 'Parceiros que fortalecem o esporte',
    'home_text_supporters' => 'Conheça as marcas que apoiam o crescimento do esporte e da nossa comunidade.',
    //home footer

    'home_footer_phrase' => 'Conectando atletas, fortalecendo o esporte.',
    'home_footer_activity' => 'Receba nossas novidades',
    'home_footer_register' => 'Cadastre-se para ficar por dentro dos próximos eventos e atualizações.',
    //FIM HOME

    //Login
    'login_title_jorney' => 'Sua jornada começa aqui!',
    'login_subtitle_jorney' => 'Conecte-se, treine e evolua com a maior comunidade de atletas.',

    'login_title_welcome' => 'Bem-vindo de volta',
    'login_subtitle_welcome' => 'Acesse sua conta para continuar',

    //Forms do login
    'login_email_ph' => 'Email',
    'login_senha_ph' => 'Senha',
    'login_remember' => 'Lembrar de mim',
    'login_forget' => 'Esqueceu a senha?',
    'login_btn_entry' => 'Entrar',

    //Login botao de cadastrar
    'login_text_register' => 'Não tem uma conta?',

    //Mensagens (Login)
    'message_password_redefinition' => 'Senha redefinida com sucesso!',
    'message_registration_success' => 'Cadastro realizado! Faça o login para continuar.',

    //ERROS (LOGIN)
    'error_incorrect_access' => 'E-mail ou senha incorretos. Tente novamente.',
    'error_invalid_token' => 'O link de redefinição é inválido ou já foi usado.',
    'error_account_inactive' => 'Esta conta está inativa. Contate o suporte.',
    'unknown_error' => 'Ocorreu um erro desconhecido.',

    //Esqueceu a senha
    'forget_title_back' => 'Vamos te colocar de volta no jogo.',
    'forget_subtitle_back' => 'Recupere seu acesso e não perca nenhuma oportunidade de se conectar.',

    'forget_title_password' => 'Esqueceu a senha?',
    'forget_subtitle_password' => 'Sem problemas! Digite seu e-mail e enviaremos um link para você criar uma nova.',

    'forget_email_registred' => 'Digite o seu E-mail cadastrado',

    //Botão para recuperar a senha
    'forget_btn_recover' => 'Enviar Link de recuperação',

    'remember_passoword_text' => 'Lembrou da senha?',

    'remember_password_btn' => 'Voltar para o login',

    //Messages (Esqueceu a senha)
    'email_exists' => 'Se um e-mail com este endereço existir, um link foi enviado.',
    //Erros

    'email_invalid' => 'Por favor, insira um e-mail válido.',
    //Fim esqueceu a senha


    //Reset password
    'reset_title' => 'Crie sua nova senha',
    'reset_passoword_one' => 'Nova senha',
    'reset_passoword_two' => 'Confirme a Nova senha',
    'reset_password_btn' => 'Salvar nova senha',

    //Erros
    'password_mismatch' => 'As senhas não conferem. Tente novamente.',
    'reset_invalid' => 'O link de redefinição é inválido ou expirou.',
    'reset_unknown_error' => 'Houve um erro ao atualizar sua senha. Tente novamente.',
    //Fim reset Password

    //Cadastro
    'register_img_title' => 'Faça parte do time.',
    'register_img_text' => 'Crie sua conta gratuita e comece a transformar sua rotina esportiva hoje mesmo.',

    'register_title_forms' => 'Crie sua conta',
    'register_subtitle_forms' => 'Preencha seus dados para começar',

    //Cadastro (forms)
    'register_name' => 'Nome Completo',
    'register_name_ph' => 'Como quer ser chamado?',
    'register_email_ph' => 'seu@email.com',
    'register_birthday' => 'Data de nascimento',
    'register_password' => 'Senha',
    'register_password_ph' => 'Mínimo de 6 caracteres',
    'register_password_two' => 'Confirmar senha',
    'register_password_ph_two' => 'Repita a senha',

    //Cadastro (Politicas)
    'register_accept_agree' => 'Eu concordo com os termos',
    'register_terms_btn' => 'Termos de uso',
    'register_privacy_btn' => 'Privacidade',

    'register_create_btn' => 'Criar conta',

    'register_confirm_answer' => 'Já tem uma conta?',
    'register_answer_btn' => 'Fazer Login',


    //Cadastro (Erros)
    'register_email_exists' => 'Este e-mail já está cadastrado. Tente fazer login.',
    'register_failed' => 'Erro ao criar conta. Tente novamente mais tarde.',
    'register_password_character' => 'A senha deve ter pelo menos 6 caracteres.',
    //Fim cadastro

    //Dashboard (Usuário)
    'dashboard_view_profile' => 'Ver perfil completo',

    'dashboard_user_greetings' => 'Olá',
    'dashboard_user_activity' => 'Aqui está o resumo das suas atividades hoje.',

    'dashboard_venue_register_title' => 'Nenhum local cadastrado',
    'dashboard_venue_register_subtitle' => 'Você ainda não tem quadras ou espaços esportivos registrados.',
    'dashboard_venue_register_btn' => 'Cadastrar primeiro local',


    //Acões do dono da quadra
    'dashboard_user_actions' => 'Ações Rápidas',

    'dashboard_user_location_title' => 'Novo Local',
    'dashboard_user_location_text' => 'Cadastre uma nova quadra',

    'dashboard_user_account_title' => 'Minha conta',
    'dashboard_user_account_text' => 'Gerenciar dados e senha',

    //Locais já cadastrados
    'dashboard_user_venues_title' => 'Meus locais',
    'dashboard_user_venue_edit' => 'Editar',

    //CNPJ

    'dashboard_cnpj_title' => 'Complete seu perfil',
    'dashboard_cnpj_text' => 'Valide seu CNPJ para começar a cadastrar e gerenciar seus locais.',
    'dashboard_cnpj_add' => 'Adicionar CNPJ',
    'dashboard_cnpj_success' => 'Parabéns! Seu CNPJ foi validado. Você já pode cadastrar locais.',
    //Fim dashboard

    //CNPJ (página de cadastro)
    'cnpj_page_title' => 'Valide seu Cadastro',
    'cnpj_page_text' => 'Para garantir a segurança da plataforma e cadastrar suas quadras, precisamos validar seu CNPJ.',
    'cnpj_title_input' => 'CNPJ da Empresa',
    'cnpj_input_ph' => '00.000.000/000-00',
    'cnpj_btn_register' => 'Validar e salvar',
    'cnpj_btn_back' => 'Voltar ao painel',
    'cnpj_kolae_safe' => '2025 Kolae. Ambiente seguro',
    //Fim CNPJ


    //Configurações

    //Menu de opções
    'config_custom_profile' => 'Editar Perfil',
    'config_security' => 'Segurança',

    //perfil
    'profile_title' => 'Informações Pessoais',
    'profile_subtitle' => 'Atualize seus dados de identificação.',
    'profile_image_desc' => 'Clique na foto para alterar',
    'profile_image_ph' => 'Alterar',
    'profile_btn_save' =>  'Salvar Alterações',

    //Mensagens
    'profile_update_success' => 'Perfil atualizado com sucesso',

    //Erros
    'profile_error_upload' => 'Erro ao fazer upload da imagem. Tente um arquivo menor (JPG/PNG).',
    'profile_error_update' => 'Erro ao atualizar o perfil.',
    'profile_error_data' => 'Não foi possível carregar os dados do usuário.',

    //Segurança
    'security_change_password_title' => 'Alterar senha',
    'security_change_password_text' => 'Mantenha sua conta segura atualizando sua senha periodicamente.',
    'security_present_password' => 'Senha Atual',
    'security_new_password' => 'Nova senha',
    'security_new_password_confirm' => 'Confirmar nova senha',
    'security_update_password_btn' => 'Salvar nova senha',


    //Erros
    'security_wrong_password_error' => 'A senha atual informada está incorreta.',
    'security_strength_password_error' => 'A senha está muito fraca.',
    'security_save_password_error' => 'Erro ao salvar. Tente novamente.',

    //Nível da senha (fraca/forte)
    'strength_password_title' => 'Força da senha',
    'strength_password_veryweak' => 'Muito fraca',
    'strength_password_weak' => 'Fraca',
    'strength_password_reasonable' => 'Razoável',
    'strength_password_strong' => 'Forte',
    'strength_password_verystrong' => 'Muito forte',
    //Fim configurações

    //Criar quadra

    //Erros
    'venue_validation_error' => 'Erro de validação',

    //Questionario 1 (Chão do local)
    'venue_floor_question' => 'Qual o tipo de piso principal do seu local?',
    'venue_floor_answer_1' => 'Grama Sintética',
    'venue_floor_answer_2' => 'Cimento',
    'venue_floor_answer_3' => 'Areia',
    'venue_floor_answer_4' => 'Saibro',
    'venue_floor_answer_5' => 'Grama Natural',
    'venue_floor_answer_6' => 'Taco',

    //Questionario 2 (Capacidade máxima)
    'venue_max_capacity' => 'Qual a capacidade da sua quadra?',
    'venue_max_capacity_desc' => 'Refere-se ao número de jogadores em campo ao mesmo tempo.',
    'venue_quantity_players' => 'Jogadores',

    //Questionario 3 (Comodidades)
    'venue_additional_title' => 'Informe aos jogadores o que seu espaço tem a oferecer',
    'venue_additional_1' => 'Iluminação',
    'venue_additional_2' => 'Quadra coberta',
    'venue_additional_3' => 'Area de lazer',
    'venue_additional_4' => 'Wi-Fi',
    'venue_additional_5' => 'Estacionamento',
    'venue_additional_6' => 'Vestiário',
    'venue_leisure_title' => 'Qual a capacidade da sua área de lazer?',
    'venue_leisure_capacity' => 'Pessoas',

    //Questionario 4 (Nome e valor por hora)
    'venue_name_price_title' => 'Agora, dê um nome e defina um preço',
    'venue_name' => 'Nome do local',
    'venue_name_ph' => 'Exemplo: Arena Kolae',
    'venue_price' => 'Preço médio por hora (R$)',
    'venue_price_ph' => 'Exemplo: 50.00',

    //Questionário 5 (localização da quadra)
    'venue_adress_title' => 'Onde fica o local?',
    'venue_cep' => 'CEP',
    'venue_street' => 'Rua',
    'venue_neighborhood' => 'Bairro',
    'venue_number' => 'Número',
    'venue_city' => 'Cidade',
    'venue_state' => 'Estado',
    'venue_complement' => 'Complemento',

    //Questionário 6 (Foto da quadra)
    'venue_image_title' => 'Faça seu local se destacar com fotos',
    'venue_image_text_1' => 'Arraste e solte suas fotos aqui',
    'venue_image_text_2' => 'ou clique para selecionar',

    'venue_btn_continue' => 'Avançar',
    'venue_btn_finish' => 'Concluir e cadastrar',

    //validação
    'venue_floor_incomplete' => 'Por favor, selecione um tipo de piso.',
    'venue_requirement' => 'Preencha os campos obrigatórios.',
    'venue_adress_incomplete' => 'Preencha o endereço completo.',
    //Fim criar quadras

    //Editar quadra
    'edit_venue_info_title' => 'Informações Básicas',
    'edit_venue_price' => 'Preço / hora',

    'edit_venue_floor_title' => 'Piso',

    'edit_venue_capacity_title' => 'Capacidade',
    'edit_venue_capacity_text' => 'Jogadores em campo',
    'edit_venue_capacity' => 'Total',

    'edit_venues_additional_title' => 'Comodidades',
    'edit_venue_answer_1' => 'Iluminação',
    'edit_venue_answer_2' => 'Cobertura',
    'edit_venue_answer_3' => 'Lazer',
    'edit_venue_capacity_leisure' => 'Capacidade da Área',

    'edit_venue_location' => 'Localização',
    'edit_venue_location_cep' => 'CEP',

    'edit_venue_photo' => 'Fotos',
    'edit_venue_photo_text_1' => 'Arraste fotos aqui',
    'edit_venue_photo_text_2' => 'ou clique para selecionar',

    'edit_venue_desativar_Text_1' => 'Desativar Quadra',
    'edit_venue_desativar_Text_2' => 'Isso removerá a quadra das buscas.',
    'edit_venue_desativar_btn' => 'Desativar',

    'edit_venue_desativar_confirm' => 'Desativar?',
    'edit_venue_desativar_alert' => 'Esta ação é irreversível. O histórico de reservas será mantido, mas a quadra ficará invisível.',
    'edit_venue_desativar_text_1' => 'Digite',
    'edit_venue_desativar_text_2' => 'para confirmar:',

    //Erros

    'edit_venue_not_found' => 'Local não encontrado',


];
